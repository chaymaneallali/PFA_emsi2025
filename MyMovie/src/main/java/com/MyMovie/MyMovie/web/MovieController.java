...
public class MovieController {
    ...
    public ResponseEntity<?> getMovieDetailsWithRecommendations(@PathVariable int movieId) {
        ...
        */
        ...
        try {
            ...
            return ResponseEntity.internalServerError()
                    .body(Map.of("error", "Failed to load movie"));
        }
    ...
    public ResponseEntity<?> getCollaborativeRecommendations(
            @PathVariable int userId,
            @RequestParam(defaultValue = "20") int top_n) {
        try {
    ...
    public ResponseEntity<?> markMovieAsWatched(
        ...
        try {
            WatchedMovie saved = recommendationService.markAsWatched(userId, movieId);
    ...
    public ResponseEntity<?> markMovieAsToWatch(
            @PathVariable int movieId,
            @RequestParam int userId) {
        try {
            ToWatchMovie saved = recommendationService.markAsToWatch(userId, movieId);
            return ResponseEntity.ok(saved);
        } catch (Exception e) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(Map.of("error", "Failed to mark as to-watch", "detail", e.getMessage()));
        }
    }
